...
debian10首先添加backports源，然后-t参数安装wireguard

```bash
apt install wireguard -t buster-backports
...
修改/etc/sysctl.conf文件将`net.ipv4.ip_forward=1`前注释去掉，没有就添加该内容
...
wg genkey | tee privatekey-_li | wg pubkey > publickey_cli
# 先用wg生成私钥，再用wg根据私钥生成公钥(base64加密)
# 服务端及每一个客户端各需要一对密钥对
```

`ip address show`查看网卡名称，一般为eth0

##### 修改配置文件

配置文件为`/etc/wireguard/wg0.conf`注意其中的网卡名称
